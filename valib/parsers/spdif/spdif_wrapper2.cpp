...
SpdifWrapper2::SpdifWrapper2(int dts_mode_, int dts_conv_)
...
bool
SpdifWrapper2::can_open(Speakers spk) const
{
...
bool
SpdifWrapper2::process(Chunk &in, Chunk &out)
{
  out = in;
  in.clear();
  if (passthrough)
...
bool
SpdifWrapper2::sync(const FrameInfo &finfo, uint8_t *frame, size_t size)
{
...
bool
SpdifWrapper2::sync_ac3(const FrameInfo &finfo, uint8_t *frame, size_t size)
{
...
bool
SpdifWrapper2::sync_eac3(const FrameInfo &finfo, uint8_t *frame, size_t size)
{
...
bool
SpdifWrapper2::sync_dts(const FrameInfo &finfo, uint8_t *frame, size_t size)
{
  int nblks = 0;
...
bool
SpdifWrapper2::sync_mpa(const FrameInfo &finfo, uint8_t *frame, size_t size)
{
  int version, layer;
...
size_t
SpdifWrapper2::wrap(const FrameInfo &finfo, uint8_t *frame, size_t size, uint8_t *dest)
{
  switch (finfo.spk.format)
  {
...
size_t
SpdifWrapper2::wrap_ac3(const FrameInfo &finfo, uint8_t *frame, size_t size, uint8_t *dest)
{
...
size_t
SpdifWrapper2::wrap_eac3(const FrameInfo &finfo, uint8_t *frame, size_t size, uint8_t *dest)
{
...
size_t
SpdifWrapper2::wrap_mpa(const FrameInfo &finfo, uint8_t *frame, size_t size, uint8_t *dest)
{
  size_t spdif_frame_size = finfo.nsamples * spdif_block_size;
  if (spdif_frame_size > max_spdif_frame_size || size > spdif_frame_size - header_size)
    return false;
